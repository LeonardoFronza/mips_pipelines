...
namespace Pipelines
{
    public class SimuladorPHT
    {
        ...
        private int pc;
        private byte[] tabelaPHP = new byte[32];

        private bool predicaoHabilitada = true;
            ...
            switch (opcode)
            {
                ...
                case "BEQ":
                    ...
                    if (this.registradores[r0] == this.registradores[r3])
                    {
                        ...
                        if (predicaoHabilitada && tabelaPHP[this.pc % 32] != 1)
                        {
                            tabelaPHP[this.pc % 32] = 1;
                            instrucoesInvalidas++;
                        }
                    }
                    else if (predicaoHabilitada && tabelaPHP[this.pc % 32] != 0)
                    {
                        tabelaPHP[this.pc % 32] = 0;
                        instrucoesInvalidas++;
                    }
                    break;
            }
        ...
        public void Run()
        {
            while (this.pc < this.instrucoes.Count)
            {
                ...
                ExibirEstatisticas();
                Console.WriteLine(DictionaryToString(this.registradores));
            }
        }

        public void DesabilitarPredicao()
        {
            this.predicaoHabilitada = false;
        }

        public void ExibirEstatisticas()
        {
            Console.WriteLine("Instruções executadas: " + instrucoesExecutadas);
            Console.WriteLine("Instruções inválidas buscadas: " + instrucoesInvalidas);
        }
